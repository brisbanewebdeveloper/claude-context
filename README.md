...
Use this when Codex is authenticated with a ChatGPT Plus/Pro/Team account and you want Claude Context to run as a separate HTTP MCP service with local Ollama embeddings. The ChatGPT login stays in Codex; the MCP server uses Ollama for embeddings and Milvus/Zilliz for vector storage.
